...
public class ChainTaller1<T extends Comparable<? super T>> extends Chain<T> {
    ...
    public boolean compare(LinearList<T> otra) {
        if (size == otra.size()) {
            for (int i = 0; i < size; i++)
                if (!get(i).equals(otra.get(i)))
                    return false;
            return true;
        } else return false;
    }
    ...
    public void eliminarRepetidos() {
        ChainTaller1<T> tmp = new ChainTaller1<>();
        int aux = 0;
        for (int i = 0; i < size; i++) {
    ...
    public void frecuenciaDeElementos() {
        ChainTaller1<T> tmp = new ChainTaller1<>();
        ChainTaller1<Integer> tmp2 = new ChainTaller1<>();
        int aux = 0;
        int aux2 = 0;
        for (T x : this)
            if (tmp.indexOf(x) == -1) {
    ...
    public ChainTaller1<T> SubList(int start, int size) {
        checkIndex(start);
    ...
    public static void main(String[] args) {
        ChainTaller1<Integer> smart = new ChainTaller1<>();
        for (int i = 0; i < 10; i++)
            smart.add(i, i * 2);
        ...
        ChainTaller1<String> smart2 = new ChainTaller1<>();
        smart2.add(0, "ABC");
        smart2.add(0, "DEF");
        smart2.add(0, "GHI");
        smart2.add(0, "JJJ");
        ...
        ChainTaller1<MyStudent> smart3 = new ChainTaller1<>();
        ...
        smart3.add(0, new MyStudent("Juant", "Sierrae", 4));
        smart3.add(0, new MyStudent("Juany", "Sierry", 6));
        smart3.add(0, new MyStudent("Juanu", "Sierrto", 7));
        smart3.add(0, new MyStudent("Juani", "Sanchexto", 8));
        smart3.add(0, new MyStudent("Juanp", "Sanck", 8));
        smart3.add(0, new MyStudent("Juand", "eeeeeeee", 1));

        ChainTaller1<MyStudent> smarter3 = new ChainTaller1<>();
